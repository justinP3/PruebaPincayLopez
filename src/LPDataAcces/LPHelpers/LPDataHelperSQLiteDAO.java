...
import LPInfrastructure.LPAppConfig;
import LPInfrastructure.LPAppException;

public class LPDataHelperSQLiteDAO<T> implements LPIDAO<T> {
    protected final Class<T> DTOClass;
    protected final String tableName;
    protected final String tablePK;

    private static final String DBPath = LPAppConfig.DATABASE;
     ...
     * @throws PjAppException: Error al asociar la clase con la tabla
     */
    public LPDataHelperSQLiteDAO(Class<T> dtoClass, String tableName, String tablePK) throws LPAppException {
        try {
            openConnection();
        } catch (SQLException e) {
            throw new LPAppException(null, e, getClass(), "DataHelperSQLiteDAO");
        }
    ...
    @Override
    public boolean create(T entity) throws LPAppException {
        ...
        try (PreparedStatement stmt = openConnection().prepareStatement(sql)) {
            ...
            throw new LPAppException(null, e, getClass(), "create");
        }
    }

    @Override
    public boolean update(T entity) throws LPAppException {
        try {
            Field[] fields = DTOClass.getDeclaredFields();
            ...
            throw new LPAppException(null, e, getClass(), "update");
        }
    }
    ...
    @Override
    public T readBy(Integer id) throws LPAppException {
    ...
    @Override
    public List<T> readAll() throws LPAppException {
    ...
    @Override
    public Integer getMaxReg() throws LPAppException {
        String sql = String.format("SELECT COUNT(*) FROM %s WHERE Estado = 'A'", tableName);
        try (PreparedStatement stmt = openConnection().prepareStatement(sql);
                ResultSet rs = stmt.executeQuery()) {
            return rs.next() ? rs.getInt(1) : 0;
        } catch (SQLException e) {
            throw new LPAppException(null, e, getClass(), "getMaxReg");
        }
    }

    protected T mapResultSetToEntity(ResultSet rs) throws LPAppException {
        try {
            ...
            throw new LPAppException(null, e, getClass(), "mapResultSetToEntity");
        }
    }
}
